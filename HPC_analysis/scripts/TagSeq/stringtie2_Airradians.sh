...
# before running..
# make directory named stringtie2 as output/stringtie2/Airradians


# call command names 
BASEDIR=~/ # base directory
REFDIR=~/refs
DATDIR=~/Airradians_F1s_TagSeq/output/hisat2/Airradians_map # directory of mapped bam files from hisat2
OUTDIR=~/Airradians_F1s_TagSeq/output/stringtie2/Airradians
...
for i in ${array[@]}; do #Running with the -e option to compare output to exclude novel genes. Also output a file with the gene abundances
	...
	stringtie -p 8 -e -B -G ${REFDIR}/Argopecten_irradians_irradians.gff -A ${OUTDIR}/${sample_name}.gene_abund.tab -o ${OUTDIR}/${sample_name}.gtf ${i}
