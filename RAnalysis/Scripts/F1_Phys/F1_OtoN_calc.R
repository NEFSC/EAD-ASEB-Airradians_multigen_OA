...
SMR_F1_sub <- SMR_F1 %>% dplyr::mutate(Date = paste("20",(format(as.Date(Date, "%m/%d/%Y"), "%y%m%d")), sep ='')) %>% # change format of the date to the format in Excretion_data
                         ...
                         dplyr::select(-(c(filetype, Length_um, pCO2,Dry_Tissue_weight, whole_Dry_weight))) %>%  # do not need it anymore!
                          dplyr::select(c('Date','pH','Replicate','Number', 'Run', 'Chamber_tank', 'Length_mm',
                                          'resp_mg_hr_bFactorNormTDW.MEAN', 'resp_umol_hr_bFactorNormTDW.MEAN'))
...
SMR_F2_sub <- SMR_F2 %>% dplyr::mutate(Date = paste("20",(format(as.Date(Date, "%m/%d/%Y"), "%y%m%d")), sep ='')) %>% # change format of the date to the format in Excretion_data
                          ...
                          dplyr::select(c('Date','pH','Replicate','Number', 'Run', 'Chamber_tank', 'Length_um',
                                         'resp_mg_hr_bFactorNormTDW.MEAN', 'resp_umol_hr_bFactorNormTDW.MEAN'))
nrow(SMR_F2_sub) # 63 
...
F1_ON_Master<- merge(ER_F1,SMR_F1_sub, by=c('Date','pH',  'Replicate','Number', 'Run')) %>% 
                  dplyr::select(-c(Chamber_tank.x,Run,Biovolume_g_in_sw,Dry_Shell_weight,whole_Dry_weight,log10_VER,log10_TDW))
nrow(F1_ON_Master) # 74 aligned - 4 less than ER master file 


F2_ON_Master <- merge(ER_F2,SMR_F2_sub, by=c('Date','pH','Replicate','Number','Chamber_tank', 'Length_um')) %>% 
                  dplyr::select(-c(Run.x,Run.y,Biovolume_g_in_sw,Dry_Shell_weight,whole_Dry_weight,log10_VER,log10_TDW))
nrow(F2_ON_Master) # 63 aligned - exact with the master file


# F1 Plots ::::::::::::::::::::::::::
library(forcats)
F1_ON_Master <- F1_ON_Master %>% 
                  dplyr::mutate(O_N =(resp_umol_hr_bFactorNormTDW.MEAN*2)/ # for umol of oxygen, curretnly as O2, convert to O 
...
F2_ON_Plot <- F2_ON_MasterMEANSMEANS %>% 
                            ...
                            theme_classic() +  
                            xlab("Age (dpf)") + 
                            ggtitle("O:N, F2 Scallops (mean +- SE; rep av'd)") +
                            theme_classic() +
                            theme(legend.position="none",
                                  panel.grid.major = element_blank(), 
                                  panel.grid.minor = element_blank())+ 
