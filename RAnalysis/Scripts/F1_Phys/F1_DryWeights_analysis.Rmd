...
F1_dryweights_calc %>% 
  ...
  ggplot(aes(Shell_length_mm,Total_Dry_Tissue_g, group = pH))+ 
    ...
    ylim(0,1.2)+
    geom_point()+
    geom_smooth(method = "lm", se=FALSE, aes(group=1, colour ="loess"),size=0.8) + geom_hline(yintercept=1, color="blue", size=1) + 
...
meanLength.all <- mean(as.numeric((F1_DryGonad %>% dplyr::filter(!Dry_Gonad_weight_g  == 0))$Shell_length_mm))# 27.22554
seLength.all   <- sd(F1_DryGonad$Shell_length_mm)/sqrt(length((F1_DryGonad$Shell_length_mm))) # 0.3582235
...
Gonad_raw_plot <- F1_DryGonad.bfactor.long.MEANS_master %>% 
                          ...
                          stat_summary(fun.min = function(x) mean(x) - sd(x)/sqrt(length(x)), 
                                        ...
                                        geom = 'errorbar', width = 0.25, size = 1,
                                        position = position_dodge2(width = 0.5)) + 
                          labs(title="Gonad: raw weight (g)", x ="Age (dpf)", y = "Dry gonad (mg no scaling)") +
                          theme_classic() +
                          theme(panel.grid.major = element_blank(), 
                                panel.grid.minor = element_blank(), 
                                axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=1),
...
Gonad_scaling_pCO2 <- F1_DryGonad.bfactor.long.MEANS_master %>% 
                          ...
                          stat_summary(fun.min = function(x) mean(x) - sd(x)/sqrt(length(x)), 
                                        ...
                                        geom = 'errorbar', width = 0.25, size = 1,
                                        position = position_dodge2(width = 0.5)) + 
                          labs(title="Gonad: Length b factor pCO2 specific", x ="Age (dpf)", y = "Dry gonad (mg Lengthbfactor_pCO2)") +
                          theme_classic() +
                          theme(panel.grid.major = element_blank(), 
                                panel.grid.minor = element_blank(), 
                                axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=1),
...
Gonad_scaling_all <- F1_DryGonad.bfactor.long.MEANS_master %>% 
...
ggarrange(Gonad_scaling_pCO2, Gonad_scaling_all)
```
...
Adductor_final_plot <- F1_DryAdductor.bfactor.long.MEANS_master %>% 
                          ...
                          stat_summary(fun.min = function(x) mean(x) - sd(x)/sqrt(length(x)), 
                                        ...
                                        geom = 'errorbar', width = 0.25, size = 1,
                                        position = position_dodge2(width = 0.5)) + 
                          labs(title="Adductor: Length b factor pCO2", x ="Age (dpf)", y = "Dry adductor (mg bFactorLength.pCO2)") +
                          theme_classic() +
                          theme(panel.grid.major = element_blank(), 
                                panel.grid.minor = element_blank(), 
                                axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=1),
...
Shell_final_plot <- F1_DryShell.bfactor.long.MEANS_master %>% 
                          ...
                          stat_summary(fun.min = function(x) mean(x) - sd(x)/sqrt(length(x)), 
                                        ...
                                        geom = 'errorbar', width = 0.25, size = 1,
                                        position = position_dodge2(width = 0.5)) + 
                          labs(title="Shell: Length b factor all", x ="Age (dpf)", y = "Dry Shell (mg Lengthbfactor_all)") +
                          theme_classic() +
                          theme(panel.grid.major = element_blank(), 
                                panel.grid.minor = element_blank(), 
                                axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=1),
...
Shell_final_plot_SUBSET <- F1_DryShell.bfactor.long.MEANS_master %>% 
                          dplyr::filter(Age >= 233 & measurement %in% 'Shell_bFactorLength.all') %>% 
                          ggplot(aes(x=factor(Age), y=value, colour=factor(pCO2)),stat="identity") +
                           scale_colour_manual(breaks=c("500 μatm", "800 μatm"), 
                                                        values=c("forestgreen","darkorange2")) +
                          stat_summary(fun.y="mean", size = 0.8,
...
Tissue_final_plot <- F1_DryTissue.bfactor.long.MEANS_master %>% 
                          ...
                          stat_summary(fun.min = function(x) mean(x) - sd(x)/sqrt(length(x)), 
                                        ...
                                        geom = 'errorbar', width = 0.25, size = 1,
                                        position = position_dodge2(width = 0.5)) + 
                          labs(title="Tissue: Length b factor all", x ="Age (dpf)", y = "Dry Tissue (mg Lengthbfactor_all)") +
                          theme_classic() +
                          theme(panel.grid.major = element_blank(), 
                                panel.grid.minor = element_blank(), 
                                axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=1),
...
Tissue_final_plot_SUBSET <- F1_DryTissue.bfactor.long.MEANS_master %>% 
                          dplyr::filter(Age >= 233 & measurement %in% 'Tissue_bFactorLength.all') %>% 
                          ggplot(aes(x=factor(Age), y=value, colour=factor(pCO2)),stat="identity") +
                           scale_colour_manual(breaks=c("500 μatm", "800 μatm"), 
                                                        values=c("forestgreen","darkorange2")) +
                          stat_summary(fun.y="mean", size = 0.8,
...
WholeDry_final_plot <- F1_WholeDry.bfactor.long.MEANS_master %>% 
                          ...
                          stat_summary(fun.min = function(x) mean(x) - sd(x)/sqrt(length(x)), 
                                        ...
                                        geom = 'errorbar', width = 0.25, size = 1,
                                        position = position_dodge2(width = 0.5)) + 
                          labs(title="Whole Dry: Length b factor all", x ="Age (dpf)", y = "Dry Tissue (mg Lengthbfactor_all)") +
                          theme_classic() +
                          theme(panel.grid.major = element_blank(), 
                                panel.grid.minor = element_blank(), 
                                axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=1),
...
WholeDry_final_plot_SUBSET <- F1_WholeDry.bfactor.long.MEANS_master %>% 
                          dplyr::filter(Age >= 233 & measurement %in% 'WholeDry_bFactorLength.all') %>% 
                          ggplot(aes(x=factor(Age), y=value, colour=factor(pCO2)),stat="identity") +
                           scale_colour_manual(breaks=c("500 μatm", "800 μatm"), 
                                                        values=c("forestgreen","darkorange2")) +
                          stat_summary(fun.y="mean", size = 0.8,
...
# Manuscript plot! 
* If we decide to go with the 'All data b factor' (not by pCO2 treatment) 
...
pdf("Output/DryWeights/F1/DryWeights_bFactorLengthAll.pdf", height=10.5, width =8)
print(ggarrange(WholeDry_final_plot_SUBSET, Gonad_final_plot, 
          Tissue_final_plot_SUBSET, Somatic_final_plot, 
          Shell_final_plot_SUBSET, Adductor_final_plot,
          ncol = 2, nrow = 3
          ))
dev.off()
