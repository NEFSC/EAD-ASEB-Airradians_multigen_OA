...
# summary: 
# TDW b factor total = 0.828
# TDW b factor pCO2 high = 0.834
# TDW b factor pCO2 low = 0.804 # two outliers removed here
...
# summary: 
# Length b factor total = 1.76
# Length b factor pCO2 high = 1.83
# Length b factor pCO2 low = 1.69
...
bLength = 1.77 # review the outlier-omitted b factor plot in previous chunk

F1s.BOXPLOT_Length_bfactor <- RR_formatted_F1s %>% 
  dplyr::filter(!Date %in% '2021-09-14') %>% 
  ...
  ggplot(aes(x = factor(Date), 
         ...
         scale_fill_manual(values=c("forestgreen","orange")) +
         theme_classic() + 
         ggtitle("shell length b factor (1.77)") +
         #ylim(0, 0.2) +
         stat_summary(fun.y=mean, 
                      geom = "errorbar", 
                      aes(ymax = ..y.., ymin = ..y..), 
...
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_multigen_OA/RAnalysis/Output/Respiration/Respiration_Boxplots_LENGTH.pdf"), width = 8, height = 8)
...
RR_formatted_F1s_OM <-  RR_formatted_F1s %>%    dplyr::filter(!Date %in% '2021-09-14')
# Two way anova for pH * time
total_mod  <- aov(lm(resp_umol_L_hr_bFactorNormLength  ~ pCO2 * Date, data = RR_formatted_F1s_OM))
leveneTest(total_mod) # 1.674e-09 ***
shapiro.test(resid(total_mod)) # < 2.2e-16
hist(resid(total_mod)) # positive skew
qqnorm(resid(total_mod))
summary(total_mod)
total_mod_transformed  <- aov(lm(log(resp_umol_L_hr_bFactorNormLength)  ~ pCO2 * Date, data = RR_formatted_F1s_OM))
leveneTest(total_mod_transformed) # 0.2057 - PASS
shapiro.test(resid(total_mod_transformed)) # 1.32e-05 - still non-normal
hist(resid(total_mod_transformed))
qqnorm(resid(total_mod_transformed))
