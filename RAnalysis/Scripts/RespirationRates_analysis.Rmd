...
* TDW b factor total = 0.822 # 0.67 (with 10/26 as 60 mL chambers)

* TDW b factor pCO2 high = 0.803 # 0.662 (with 10/26 as 60 mL chambers)

* TDW b factor pCO2 low = 0.841 - two outliers removed here # 0.678 (with 10/26 as 60 mL chambers)
...
nrow(RR_master_OM) # 77 - same as before - there are no dry weights for the animals that we do not have biovolume for, thus these are the same datasets
...
# OUTPUT PLOTS 
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_multigen_OA/RAnalysis/Output/Respiration/metabolic_scaling/MetabolicScaling_bFactor_TDW_biovolcalc.pdf"), width = 8, height = 8)
print(ggarrange(MO2_b.factor_PLOT, MO2_b.factor_PLOT_facetted, nrow = 2, ncol = 1)) # print the model diagnostics
dev.off() 
...
* Length b factor total = 2.18 # 2.13 (with 10/26 as 60 mL chambers)

* Length b factor pCO2 high = 2.2 # 2.13 (with 10/26 as 60 mL chambers)

* Length b factor pCO2 low = 2.16 # 2.14 (with 10/26 as 60 mL chambers)
...
bTDW = 0.671 # review the outlier-omitted b factor plot in previous chunk
bLength = 2.13 # review the outlier-omitted b factor plot in previous chunk


medianLength <- median(RR_formatted_F1s$Length_um/1000) # 12.07 mm
...
meanLength   <- mean(RR_formatted_F1s$Length_um/1000) # 16.57877 mm
...
RR_formatted_F1smaster <- RR_formatted_F1s %>% 
  ...
  dplyr::mutate(resp_umol_hr_bFactorNormTDW.MEAN = 
                  (resp_umol_hr)*((meanTDW/Dry_Tissue_weight)^bTDW)) %>% # TDW b factor - using 'resp_umol_hr' only values with measured biovolume
  ...
  dplyr::mutate(resp_mg_hr_bFactorNormTDW.MEAN = 
                  (resp_mg_hr)*((meanTDW/Dry_Tissue_weight)^bTDW)) %>% # TDW b factor - using 'resp_umol_hr' only values with measured biovolume
