...
# examine biovolume
# objective: we do not have biovolume for all samples 
# measured for RR, however we need this information to 
# subtract displacement of vol from the RR chambers 
# we do have length for all samples, can we estimate biovol with this info?
```{r, biovolume calc & alternatives}
...
Length.biovol <- Gurr2021_size.relationships[!is.na(Gurr2021_size.relationships$length.biov),] %>% summarySE(measurevar="length.biov")  # mean = 2.55471	stdev = 0.4976332	- highly variable relationship

Length.Width <- Gurr2021_size.relationships[!is.na(Gurr2021_size.relationships$length.width),] %>% summarySE(measurevar="length.width")  # mean = 0.9387522	stdev = 0.02717396	- consistent relationship

Length.Thickness <- Gurr2021_size.relationships[!is.na(Gurr2021_size.relationships$length.thick),] %>% summarySE(measurevar="length.thick")  # mean = 2.487012	stdev = 0.2249636	- highly variable relationship
...
```{r shell lengths at each timepoint}
...
# summary: 
# TDW b factor total = 0.712
# TDW b factor pCO2 high = 0.699
# TDW b factor pCO2 low = 0.725 # two outliers removed here
...
# summary: 
# Length b factor total = 2.24
# Length b factor pCO2 high = 2.25
# Length b factor pCO2 low = 2.24
...
# WDW b factor total = 0.776
# WDW b factor pCO2 high = 0.769
# WDW b factor pCO2 low = 0.787
...
## BOXPLOTS by TISSUE DRY WEIGHT (b factor = 0.863)
```{r, by mg TISSUE}
...
bTDW = 0.712 # review the outlier-omitted b factor plot in previous chunk
...
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_multigen_OA/RAnalysis/Output/Respiration/Respiration_Boxplots_TDW.pdf"), width = 8, height = 8)
...
## BOXPLOTS by SHELL LENGTH (b factor = 1.77)
```{r, by MM LENGTH}
...
bLength = 2.24 # review the outlier-omitted b factor plot in previous chunk
...
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_multigen_OA/RAnalysis/Output/Respiration/Respiration_Boxplots_LENGTH.pdf"), width = 18, height = 8)
...
## BOXPLOTS by WHOLE DRY WEIGHT (b factor = 0.611)
```{r, by mg WHOLE DRY WEIGHT}
...
bWDW = 0.776 # review the outlier-omitted b factor plot in previous chunk
# View(RR_formatted)
F1s.BOXPLOT_WDW_bfactor <- RR_formatted_F1s %>% 
  ...
  ggplot(aes(x = factor(Date), 
         ...
         scale_fill_manual(values=c("forestgreen","orange")) +
         theme_classic() + 
         ggtitle("whole Dry weight b factor (0.776)") +
         #ylim(0, 0.2) +
         stat_summary(fun.y=mean, 
                      geom = "errorbar", 
                      aes(ymax = ..y.., ymin = ..y..), 
...
for (i in 1:nrow(ANOVA_Dates)) {
  ...
  data_loop     <- RR_formatted_F1s %>% 
                          ...
                          dplyr::select(Date, Age, pCO2, resp_umol_L_hr_bFactorNormWDW) %>% 
                          na.omit()

        AOVmod              <- aov(lm(data_loop$resp_umol_L_hr_bFactorNormWDW ~ data_loop$pCO2))
...
write.csv(AOVdf_total, "C:/Users/samjg/Documents/Github_repositories/Airradians_multigen_OA/RAnalysis/Output/Respiration/Respiration_ANOVA_table_WDW.csv")
...
total_mod  <- aov(lm(resp_umol_L_hr_bFactorNormWDW  ~ pCO2 * Date, data = RR_formatted_F1s))
leveneTest(total_mod) # 1.674e-09 ***
shapiro.test(resid(total_mod)) # < 2.2e-16
hist(resid(total_mod)) # positive skew
qqnorm(resid(total_mod))
total_mod_transformed  <- aov(lm(log(resp_umol_L_hr_bFactorNormLength)  ~ pCO2 * Date, data = RR_formatted_F1s))
leveneTest(total_mod_transformed) # 0.2057 - PASS
shapiro.test(resid(total_mod_transformed)) # 1.32e-05 - still non-normal
hist(resid(total_mod_transformed))
qqnorm(resid(total_mod_transformed))
summary(total_mod_transformed)
TukeyHSD(total_mod_transformed)
#find optimal lambda for Box-Cox transformation 
library(MASS)
bc     <- boxcox(RR_formatted_F1s$resp_umol_L_hr_bFactorNormWDW ~ RR_formatted_F1s$pCO2 * RR_formatted_F1s$Date) # y ~ x
lambda <- bc$x[which.max(bc$y)] # 0.1414141
new_model <- lm(((resp_umol_L_hr_bFactorNormWDW^lambda-1)/lambda) ~ pCO2 * Date, data = RR_formatted_F1s) #fit new linear regression model using the Box-Cox transformation
leveneTest(new_model) # 0.04959 * - PASS
shapiro.test(resid(new_model)) # 0.002069 - still non-normal
...
``` {r stats F1s, Length b factor norm}
unique(RR_formatted_F1s$Date) # 2021-09-14 2021-09-30 2021-10-26 2022-02-02 2022-03-01 2022-09-22
# f1s data
RR_formatted_F1s <- RR_formatted %>% dplyr::filter(!Gen %in% 'F2s') %>% 
  dplyr::mutate(resp_umol_L_hr_bFactorNormLength = resp_umol_L_hr/( (Length_um*100)^bLength)) %>% 
  dplyr::mutate(Date = as.factor(Date))
...
for (i in 1:nrow(ANOVA_Dates)) {
  ...
  data_loop     <- RR_formatted_F1s %>% 
                          dplyr::filter(Date == date_loop) %>% 
                          dplyr::select(Date, Age, pCO2, resp_umol_L_hr_bFactorNormLength) %>% 
                          na.omit()
...
# WRITE CSV OF THE MASTER FILE
write.csv(AOVdf_total, "C:/Users/samjg/Documents/Github_repositories/Airradians_multigen_OA/RAnalysis/Output/Respiration/Respiration_ANOVA_table_Length.csv")
#write.csv(Biodep_Master, "C:/Users/samuel.gurr/Documents/Github_repositories/Airradians_multigen_OA/RAnalysis/Output/Biodeposition/Biodeposition_master.csv")


# Two way anova for pH * time
total_mod  <- aov(lm(resp_umol_L_hr_bFactorNormLength  ~ pCO2 * Date, data = RR_formatted_F1s))
leveneTest(total_mod) # 0.0625 
shapiro.test(resid(total_mod)) # 0.1162
hist(resid(total_mod)) # positive skew
qqnorm(resid(total_mod))
summary(total_mod)
#              Df    Sum Sq   Mean Sq F value  Pr(>F)   
# pCO2          1 1.179e-27 1.179e-27   5.375 0.02213 * 
# Date          6 5.217e-27 8.694e-28   3.963 0.00119 **
# pCO2:Date     6 1.029e-27 1.715e-28   0.782 0.58586   
# Residuals   119 2.610e-26 2.194e-28  


#find optimal lambda for Box-Cox transformation 
library(MASS)
bc     <- boxcox(RR_formatted_F1s_OM$resp_umol_L_hr_bFactorNormLength ~ RR_formatted_F1s_OM$pCO2 * RR_formatted_F1s_OM$Date) # y ~ x
lambda <- bc$x[which.max(bc$y)] # 0.1414141
new_model <- lm(((resp_umol_L_hr_bFactorNormLength^lambda-1)/lambda) ~ pCO2 * Date, data = RR_formatted_F1s_OM) #fit new linear regression model using the Box-Cox transformation
leveneTest(new_model) # 0.04959 * - PASS
shapiro.test(resid(new_model)) # 0.002069 - still non-normal
qqnorm(resid(new_model)) # boxcox resolves the qqnorm visual from the raw model greatly
summary(aov(new_model))  # date and pH significant 
TukeyHSD(aov(new_model)) 
