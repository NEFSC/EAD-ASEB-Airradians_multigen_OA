...
RR_master_without_TDW                   <- RR_master[is.na(RR_master$Dry_Tissue_weight),] %>% filter(!Fed_Unfed %in% 'Low food') # call all data with 'NA' instead of TDW value
-12.6+ 3.37(x) = y
...
ALL.BOXPLOT_facetted <- RR_formatted %>% 
  ggplot(aes(x = factor(Date), 
             y = resp_µmol_L_mg_TDW_hr, 
             fill = pCO2)) +
         geom_boxplot(alpha = 0.5, # color hue
...
ALL.BOXPLOT_facetted


# PLOT THE F2S DATA FOR TISSUE DRY WEIGHT CORRECTION
...
b = 0.863 # review the outlier-omitted b factor plot in previous chunk

F1s.BOXPLOT_facetted_bfactor <- RR_formatted %>% 
  dplyr::filter(!Gen %in% 'F2s') %>% 
  dplyr::mutate(resp_umol_L_hr_bFactorNorm = resp_umol_L_hr/(Dry_Tissue_weight^b)) %>% 
  ggplot(aes(x = factor(Date), 
         ...
         stat_summary(fun.y=mean, 
                      ...
                      position = position_dodge(preserve = "single")) +
         facet_wrap(~Gen_lifestage, scales = "free")
F1s.BOXPLOT_facetted_bfactor # not that we do not have TDW for 9/14 and 9/30 data 
