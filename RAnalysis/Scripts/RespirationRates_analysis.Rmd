...
RR_master <- read.csv(file="Output/Respiration/Calculated_Resp_Master.csv", header=T) %>% 
              ...
              dplyr::mutate(resp_umol_L_hr_mg_WDW = 
                            resp_umol_L_hr/as.numeric(whole_Dry_weight)) %>%  #  whole dry weight corrected MO2
...
print(ggarrange(F1s.BOXPLOT_TDW_facetted, nrow = 1, ncol = 1)) # print the model diagnostics 
dev.off()
...
F1s.BOXPLOT_Length_bfactor.MEDIAN.nofacet <- RR_formatted_F1s.length %>% 
  ggplot(aes(x = factor(Date), 
         ...
         scale_fill_manual(values=c("forestgreen","orange")) +
         theme_classic() + 
         ggtitle("Resp * [ ( medianLength / Length_meas)^2.22 ]") +
         #ylim(0, 0.2) +
         stat_summary(fun.y=mean, 
                      geom = "errorbar", 
                      aes(ymax = ..y.., ymin = ..y..), 
