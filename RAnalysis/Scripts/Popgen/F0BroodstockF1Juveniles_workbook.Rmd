...
knitr::opts_knit$set(root.dir = "C:/Users/samjg/Documents/Github_repositories/Airradians_multigen_OA/HPC_analysis") # Sam's
# knitr::opts_knit$set(root.dir = "C:/Users/samuel.gurr/Documents/Github_repositories/EAD-ASEB-Airradians_multigen_OA/HPC_analysis") # Sam's
>>>>>>> 330d1b4c2e0f8f9e7342ae770fbf6f9a65be4880
=======
knitr::opts_knit$set(root.dir = "C:/Users/samjg/Documents/Github_repositories/Airradians_multigen_OA/HPC_analysis") # Sam's
# knitr::opts_knit$set(root.dir = "C:/Users/samuel.gurr/Documents/Github_repositories/EAD-ASEB-Airradians_multigen_OA/HPC_analysis") # Sam's
=======
# knitr::opts_knit$set(root.dir = "C:/Users/samjg/Documents/Github_repositories/Airradians_multigen_OA/HPC_analysis") # Sam's
knitr::opts_knit$set(root.dir = "C:/Users/samuel.gurr/Documents/Github_repositories/EAD-ASEB-Airradians_multigen_OA/HPC_analysis") # Sam's
>>>>>>> 96298a93187dbf2d945062bca529290100b65058
>>>>>>> 214fa44b371a13bca2ef83b7d5131affc2c7a2c5
...
F0B_F1JALL_intersect.vcf     <- read.vcf(paste0(path,"F0F1_intersect.vcf.gz"), from = NULL, to = NULL, which.loci = NULL, quiet = FALSE)
write.vcf(F0B_F1JALL_intersect.vcf, paste0(path,"F0F1_intersect.vcf"), CHROM = NULL, POS = NULL, quiet = FALSE)
vcf2ploidy_app()


F0B_F1JALL_intersect.bed     <- read.pcadapt(paste0(path,"F0F1_intersect.bed"), type = "bed") # 6,048 variants

F0B_F1JALL_intersect_longcontigs.vcf     <- read.vcfR(paste0(path,"F0F1_intersect_longcontigs.vcf.gz")) # 6,048 variants
F0B_F1JALL_intersect_longcontigs.bed     <- read.pcadapt(paste0(path,"F0F1_intersect_longcontig.bed"), type = "bed") # 6,048 variants

# MERGED: ALL f0 broodstock and f1 juveniles from Low and Moderate OA
...
# SEPARATE merge 0f F0 Broodstock and F1 JUVENILES of different pCO2
## LOW (pH8)
...
F0BroodF1Juveniles_strata   <- read.csv("output/lcWGS/angsd/IDs/F0BroodstockF1Juveniles.csv", sep =',', header=FALSE) %>% 
                                ...
                                dplyr::mutate(Gen_Treatment = dplyr::case_when(Gen == "F0" ~ "F0",
                                                                        Gen == "F1" ~ paste0(Gen,'_',Treatment)))


# subsets  mmmkay?

F0F1_strata_pH75           <- F0BroodF1Juveniles_strata %>% dplyr::filter(!Treatment %in% 'Low') # keep modrate, pH 75
F0F1_strata_pH8            <- F0BroodF1Juveniles_strata %>% dplyr::filter(!Treatment %in% 'Moderate') # keep modrate, pH 75
```
...
F0B_F1JuvAll_intersect.diagnostic      <- pcadapt(F0B_F1JALL_intersect.bed, K = 20)
plot(F0B_F1JuvAll_intersect.diagnostic, option = "screeplot") # looks like 7
F0B_F1JuvAll_intersect.res             <- pcadapt(F0B_F1JALL_intersect.bed, K = 7) # assign
plot(F0B_F1JuvAll_intersect.res, option = "manhattan")

## F0 brood with all F1 juvneiles , LONG CONTIGS ONLY
F0B_F1JuvAll_intersect_longcontigs.diagnostic      <- pcadapt(F0B_F1JALL_intersect_longcontigs.bed, K = 20)
plot(F0B_F1JuvAll_intersect_longcontigs.diagnostic, option = "screeplot") # looks like 7
F0B_F1JuvAll_intersect_longcontigs.res             <- pcadapt(F0B_F1JALL_intersect_longcontigs.bed, K = 7) # assign
plot(F0B_F1JuvAll_intersect_longcontigs.res, option = "manhattan")

# look at both 
ggarrange(
  plot(F0B_F1JuvAll_intersect.res, option = "manhattan"),
  plot(F0B_F1JuvAll_intersect_longcontigs.res, option = "manhattan"), nrow = 2
)
```

```{r screeplot, Merge F0 broodstock F1 juveniles}

# MERGED DATA ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
## data: F0B_F1JALL_merged.bed, F0B_F1JLOW_merged.bed, F0B_F1JMODERATE_merged.bed
F0B_F1JuvAll_merge.diagnostic          <- pcadapt(F0B_F1JALL_merged.bed, K = 20)
plot(F0B_F1JuvAll_merge.diagnostic, option = "screeplot") # looks like 6
F0B_F1JuvAll_merge.res                 <- pcadapt(F0B_F1JALL_merged.bed, K = 6) # assign
plot(F0B_F1JuvAll_merge.res, option = "manhattan")


## * F0 Broodstock w/ F1 Juveniles LOW
F0B_F1JuvLOW_merge.diagnostic          <- pcadapt(F0B_F1JLOW_merged.bed, K = 20)
plot(F0B_F1JuvLOW_merge.diagnostic, option = "screeplot") # looks like 6
F0B_F1JuvLOW_merge.res                 <- pcadapt(F0B_F1JLOW_merged.bed, K = 6) # assign
plot(F0B_F1JuvLOW_merge.res, option = "manhattan")


## * F0 Broodstock w/ F1 Juveniles MODERATE
F0B_F1JMODERATE_merged.diagnostic     <- pcadapt(F0B_F1JMODERATE_merged.bed, K = 20)
plot(F0B_F1JMODERATE_merged.diagnostic, option = "screeplot") # looks like 3
F0B_F1JuvMODERATE_merge.res           <- pcadapt(F0B_F1JMODERATE_merged.bed, K = 3) # assign
plot(F0B_F1JuvMODERATE_merge.res, option = "manhattan")
```
...
par(mfrow = c(2, 2))
for (i in 1:4) 
  plot(F0B_F1JuvAll_intersect.res$loadings[, i], pch = 19, cex = .3, ylab = paste0("Loadings PC", i))
...
par(mfrow = c(2, 2))
for (i in 1:4) 
  plot(F0B_F1JuvAll_merge.res$loadings[, i], pch = 19, cex = .3, ylab = paste0("Loadings PC", i))
...
test <- as.data.frame(clump4me(data         = F0B_F1JALL_intersect.bed,
                             LD.thr         = 0.1, # r squares
                             LD.K           = 4, # number of PCAs
                             LD.minMaf      = 0.05, # minumum allele freq threshold (5% - =0.05, etc)
                             outputfilename = F0B_F1JALL_intersect_LD))
plot(output_PC1~input_Window , data=test) # window of 250
F0B_F1JuvAll_intersect.LDdiagnostics <- pcadapt(F0B_F1JALL_intersect.bed, 
                                                K = 20, LD.clumping = list(size = 250, thr = 0.1),
                                                min.maf= 0.05)
plot(F0B_F1JuvAll_intersect.LDdiagnostics, option = "screeplot") # Lets try and use K = 7 after SNP thinning 
F0B_F1JuvAll_intersect.LDres <- pcadapt(F0B_F1JALL_intersect.bed, 
                                        K = 7, 
                                        LD.clumping = list(size = 250, thr = 0.1), 
                                        min.maf= 0.05)
ggarrange( (plot(F0B_F1JuvAll_intersect.res, option = "manhattan")),
           (plot(F0B_F1JuvAll_intersect.LDres, option = "manhattan")),nrow=2)


# MERGED DATA ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# all
# *  run the bam file - sliding window 50 -1000 at settings
test <- as.data.frame(clump4me(data         = F0B_F1JALL_merged.bed,
                             LD.thr         = 0.1, # r squares
                             LD.K           = 4, # number of PCAs
                             LD.minMaf      = 0.05, # minumum allele freq threshold (5% - =0.05, etc)
                             outputfilename = F0B_F1JALL_merged))
plot(output_PC1~input_Window , data=test) # window of 500
F0B_F1JuvAll_merged.LDdiagnostics <- pcadapt(F0B_F1JALL_merged.bed, K = 20, 
                                             LD.clumping = list(size = 500, thr = 0.1),
                                             min.maf= 0.05)
plot(F0B_F1JuvAll_merged.LDdiagnostics, option = "screeplot") # Lets try and use K = 4 after SNP thinning 
F0B_F1JuvAll_merged.LDres <- pcadapt(F0B_F1JALL_merged.bed, 
                                        K = 4, 
                                        LD.clumping = list(size = 500, thr = 0.1),
                                        min.maf=0.05)
ggarrange( (plot(F0B_F1JuvAll_merge.res, option = "manhattan")),
           (plot(F0B_F1JuvAll_merged.LDres, option = "manhattan")),nrow=2)
...
# pH 8 - LOW
# *  run the bam file - sliding window 50 -1000 at settings
test <- as.data.frame(clump4me(data         = F0B_F1JLOW_merged.bed,
                             LD.thr         = 0.1, # r squares
                             LD.K           = 4, # number of PCAs
                             LD.minMaf      = 0.05, # minumum allele freq threshold (5% - =0.05, etc)
                             outputfilename = F0B_F1JLOW_merged))
plot(output_PC1~input_Window , data=test) # window of 950
F0B_F1JLOW_merged.LDdiagnostics <- pcadapt(F0B_F1JLOW_merged.bed, K = 20, 
                                             LD.clumping = list(size = 950, thr = 0.1),
                                             min.maf= 0.05)
plot(F0B_F1JLOW_merged.LDdiagnostics, option = "screeplot") # Lets try and use K = 2 after SNP thinning 
F0B_F1JLOW_merged.LDres <- pcadapt(F0B_F1JLOW_merged.bed, 
                                        K = 2, 
                                        LD.clumping = list(size = 950, thr = 0.1),
                                        min.maf=0.05)
...
# pH 75 - MODERATE
# *  run the bam file - sliding window 50 -1000 at settings
test <- as.data.frame(clump4me(data         = F0B_F1JMODERATE_merged.bed,
                             LD.thr         = 0.1, # r squares
                             LD.K           = 4, # number of PCAs
                             LD.minMaf      = 0.05, # minumum allele freq threshold (5% - =0.05, etc)
                             outputfilename = F0B_F1JMODERATE_merged))
plot(output_PC1~input_Window , data=test) # window of 300
F0B_F1JuvMODERATE_merged.LDdiagnostics <- pcadapt(F0B_F1JMODERATE_merged.bed, K = 20, 
                                             LD.clumping = list(size = 300, thr = 0.1),
                                             min.maf= 0.05)
plot(F0B_F1JuvMODERATE_merged.LDdiagnostics, option = "screeplot") # Lets try and use K = 4 after SNP thinning 
F0B_F1JuvMODERATE_merged.LDres <- pcadapt(F0B_F1JMODERATE_merged.bed, 
                                        K = 6, 
                                        LD.clumping = list(size = 300, thr = 0.1),
                                        min.maf=0.05)
ggarrange( (plot(F0B_F1JuvMODERATE_merge.res, option = "manhattan")),
           (plot(F0B_F1JuvMODERATE_merged.LDres, option = "manhattan")),nrow=2)
```
