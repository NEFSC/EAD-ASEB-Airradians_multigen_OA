...
F0B_F1JALL_intersect.vcf     <- read.vcf(paste0(path,"F0F1_intersect.vcf.gz"), 
                                         from = NULL, to = NULL, which.loci = NULL, quiet = FALSE)
write.vcf(F0B_F1JALL_intersect.vcf, paste0(path,"F0F1_intersect.vcf"), 
          CHROM = NULL, POS = NULL, quiet = FALSE)
vcf2ploidy_app()


F0B_F1JALL_intersect.bed     <- read.pcadapt(paste0(path,"F0F1_intersect.bed"), type = "bed") # 6,048 variants

F0B_F1JALL_intersect_longcontigs.vcf     <- read.vcfR(paste0(path,"F0F1_intersect_longcontigs.vcf.gz")) # 2,920  variants
F0B_F1JALL_intersect_longcontigs.bed     <- read.pcadapt(paste0(path,"F0F1_intersect_longcontig.bed"), type = "bed") # 2,920  variants
...
```{r build genlight}
?sequoia
F0BF1J_longcontigs_genlight <- vcfR2genlight(F0B_F1JALL_intersect_longcontigs.vcf, n.cores = 4)
...
F0BroodF1Juveniles_strata   <- read.csv("output/lcWGS/angsd/IDs/F0BroodstockF1Juveniles.csv", sep =',', header=FALSE) %>% 
                                ...
                                dplyr::mutate(Individual = gsub('*./','',Individual),
                                      ...
                                      grepl(c("pH75|201|203|204|251|253|254|301|303|304|351|352|353|354"), Individual) ~
                                        "Moderate",
                                      grepl(c("pH8|101|103|104|153|154|155|3_||.4_|.5_"), Individual) ~ 
                                        "Low")) %>% 
