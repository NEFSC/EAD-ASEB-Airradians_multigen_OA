...
F1F3_merged.vcf        <- read.vcfR(paste0(path,"F1F3_LOWvMOD_merge.vcf.gz")) # 96,703 variants
#F1F3_merged.vcf_tidy   <- vcfR2tidy(F1F3_merged.vcf)
F1F3_merged.bed        <- read.pcadapt(paste0(path,"F1F3_LOWvMOD.bed"), type = "bed") # 96,703 variants

# MERGED: ALL f1 and f3 juveniles from Low and Moderate OA, LONG CONTIGS ONLY

F1F3_merged_longcontigs.vcf        <- read.vcfR(paste0(path,"F1F3_LOWvMOD_merge_longcontigs.vcf.gz")) # 48,120 variants
# F1F3_merged_longcontigs.vcf_tidy   <- vcfR2tidy(F1F3_merged_longcontigs.vcf)
F1F3_merged_longcontigs.bed        <- read.pcadapt(paste0(path,"F1F3_LOWvMOD_merge_longcontig.bed"), type = "bed") # 48,120 variants
...
F1F3_merged.diagnostic      <- pcadapt(F1F3_merged.bed, K = 20)
plot(F1F3_merged.diagnostic, option = "screeplot") # looks like 7
F1F3_merged.res             <- pcadapt(F1F3_merged.bed, K = 5) # assign
plot(F1F3_merged.res, option = "manhattan")

## F1 and F3 Juveniles, all data
## data: F1F3_merged_longcontigs.bed
F1F3_merged_longcontigs.diagnostic      <- pcadapt(F1F3_merged_longcontigs.bed, K = 20)
plot(F1F3_merged_longcontigs.diagnostic, option = "screeplot") # looks like 7
F1F3_merged_longcontigs.res             <- pcadapt(F1F3_merged_longcontigs.bed, K = 5) # assign
...
clump4me <- function(data, LD.thr, LD.K, LD.minMaf, outputfilename) {
        ...
        loop.table          <- data.frame(matrix(nrow = 1, ncol = 14)) # create dataframe, columns for the number of sampling days
        colnames(loop.table)<-c('output_PC1', 'output_PC2', 'output_PC3', 'output_PC4', 'output_PC5', 'output_PC6',
                                'output_PC7', 'output_PC8', 'output_PC9','output_PC10',
                                'input_MinMaf', 'input_Window', 'input_Threshold', 'input_NumPCAs') # names for comuns in the for loop
        LD.window_loop      <- seq(100,6000, by=200) # run the loop for CPM from 1 - 10 (counts per million) 
        ...
        for (i in 1:nrow(LD.window_loop)) {
          ...
          loop.table$output_PC2      = abs(mean(res$scores[,2]))
          loop.table$output_PC3      = abs(mean(res$scores[,3]))
          loop.table$output_PC4      = abs(mean(res$scores[,4]))
          loop.table$output_PC5      = abs(mean(res$scores[,5]))
          loop.table$output_PC6      = abs(mean(res$scores[,6]))
          loop.table$output_PC7      = abs(mean(res$scores[,7]))
          loop.table$output_PC8      = abs(mean(res$scores[,8]))
          loop.table$output_PC9      = abs(mean(res$scores[,9]))
...
F1F3_merged.test <- as.data.frame(clump4me(data         = F1F3_merged.bed,
                             ...
                             LD.K           = 10, # number of PCAs
                             LD.minMaf      = 0.05, # minumum allele freq threshold (5% - =0.05, etc)
                             outputfilename = F1F3_merged_LD))
plot(output_PC1~input_Window, data=F1F3_merged.test) # window of 4500
F1F3_merged.LDdiagnostics <- pcadapt(F1F3_merged.bed, 
                                                K = 20, LD.clumping = list(size = 4500, thr = 0.1),
                                                min.maf= 0.05)
plot(F1F3_merged.LDdiagnostics, option = "screeplot") # Lets try and use K = 7 after SNP thinning 
F1F3_merged.LDres <- pcadapt(F1F3_merged.bed, 
                                        K = 2, 
...
F1F3_merged_long_contigs.test <- as.data.frame(clump4me(data = F1F3_merged_longcontigs.bed,
                                     ...
                                     LD.minMaf      = 0.05, # minumum allele freq threshold (5% - =0.05, etc)
                                     outputfilename = F1F3_merged_LD))
plot(output_PC1~input_Window, data=F1F3_merged_long_contigs.test) # window of 250
F1F3_merged_long_contigs.LDdiagnostics <- pcadapt(F1F3_merged_longcontigs.bed, 
                                                K = 20, LD.clumping = list(size = 550, thr = 0.1),
                                                min.maf= 0.05)
plot(F1F3_merged_long_contigs.LDdiagnostics, option = "screeplot") # Lets try and use K = 7 after SNP thinning 
F1F3_merged_longcontigs.LDres <- pcadapt(F1F3_merged_longcontigs.bed, 
                                        K = 6, 
                                        LD.clumping = list(size = 550, thr = 0.1), 
                                        min.maf= 0.05)
ggarrange( (plot(F1F3_merged_longcontigs.res, option = "manhattan")),
           (plot(F1F3_merged_longcontigs.LDres, option = "manhattan")),nrow=2)
...
F1F3_longcontigs_LDfiltered.vcf  <- F1F3_merged_longcontigs.vcf[!is.na(F1F3_merged_longcontigs.LDres$loadings[,1]),] # 48,120 DOWN TO 7,424 variants

length(F1F3_merged_longcontigs.LDres$loadings[!is.na(F1F3_merged_longcontigs.LDres$loadings[,1]),])
```
...
Longcontigs_qqmanResults_qqman    <- F1F3_longcontigs_qqman %>% 
                                          dplyr::mutate(
                                            ...
                                            CHR = as.numeric(extract_numeric(F1F3_LD_tidy_longcontigs$fix$CHROM)),
