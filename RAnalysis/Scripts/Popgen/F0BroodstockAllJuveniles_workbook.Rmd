...
Rsamtools::bcfSamples(paste0(
    path,
    "F0_Broodstock_all_juveniles/F0_OM_all_juveniles_doMaf1_minMaf0.05_majorminor4_minind321_minD5x1605_maxD20x6420minDind5_maxDind150_minq30_minmapQ.bcf"))

x <- Rsamtools::scanBcf(paste0(
    path,
    "F0_Broodstock_all_juveniles/F0_OM_all_juveniles_doMaf1_minMaf0.05_majorminor4_minind321_minD5x1605_maxD20x6420minDind5_maxDind150_minq30_minmapQ.bcf"))
library(vcfR)
library(hierfstat)
...
```{r downloaded plink to run locally}

# run plink locally 
# I ran on SEDNA not with the --mind function to omitt samples with low data
# I received an error below iwth pcadapt because the samples have missing data
# below I used --mind to omitt the 3 poblematic files
# I discovred there were 3 bad files from the GenoConvert call in the se  uoia pipeline below
setwd("C:/Users/samjg/Documents")
runPLINK <- function(PLINKoptions = "") system(paste("C:/Users/samjg/Documents/Bioinformatics/plink_win32_20241013/plink", PLINKoptions))
runPLINK("--vcf Github_repositories/Airradians_multigen_OA/HPC_analysis/output/lcWGS/angsd/F0_Broodstock_all_juveniles/F0_OM_all_juveniles_doMaf1_minMaf0.05_majorminor4_minind321_minD5x1605_maxD20x6420minDind5_maxDind150_minq30_minmapQ.vcf.gz --double-id --allow-extra-chr --set-missing-var-ids @:# --mind 0.8 --make-bed --pca --out Github_repositories/Airradians_multigen_OA/HPC_analysis/output/lcWGS/angsd/F0_Broodstock_all_juveniles/F0_OM_all_juveniles_plink")
...
path = "output/lcWGS/angsd/"

# INTERSECT: ALL f0 broodstock and f1 juveniles from Low and Moderate OA
F0B_AllJuveniles.vcf     <-read.vcfR(
  paste0(
    path,
    "F0_Broodstock_all_juveniles/F0_OM_all_juveniles_doMaf1_minMaf0.05_majorminor4_minind321_minD5x1605_maxD20x6420minDind5_maxDind150_minq30_minmapQ.vcf.gz")
  ) # 3,121  variants
# F0B_AllJuveniles.vcf     <- read.vcf(paste0(path,"F0_OM_all_juveniles_doMaf1_minMaf0.05_majorminor4_minind321_minD5x1605_maxD20x6420minDind5_maxDind150_minq30_minmapQ.vcf.gz"), 
...
F0B_AllJuveniles.bed     <- read.pcadapt(
                                          paste0(
                                            path,
                                            "F0_Broodstock_all_juveniles/F0_OM_all_juveniles_plink.bed"), 
                                          type = "bed") # 3121 variants
...
### build genlight 
* why? diagnose whethere there are issues with missingness or low SNP calls, 
GenoConvert tells us thee diagnostics
```{r diagnostics with genelight and GonConvert}
# create genelight objet
F0BAllJuvs_genlight <- vcfR2genlight(F0B_AllJuveniles.vcf, n.cores = 4)
GenoConvert(InData = F0BAllJuvs_genlight,
            # InFile = NULL,
            InFormat = "seq",
            # OutFile = NA,
            OutFormat = "seq")
...
Geno_Sequoia <- GenoConvert(
                    InData = F0BAllJuvs_genlight,
                    # InFile = NULL,
                    InFormat = "seq",
                    # OutFile = NA,
                    OutFormat = "seq")
rownames(Geno_Sequoia)
?GenoConvert
# convert to matrix
Geno_Sequoia                <-  (as.matrix(F0BAllJuvs_genlight))
...
F0BroodF1Juveniles_strata   <- read.csv("output/lcWGS/angsd/IDs/F0BroodstockF1Juveniles.csv", 
                                        sep =',', 
                                        header=FALSE) %>% 
...
```{r screeplot, Intersect F0 broodstock all juveniles}
?pcadapt
# run pca to determine the best value k to use in the analysis
F0B_AllJuv.diagnostic      <- pcadapt(input=F0B_AllJuveniles.bed, K=20)
plot(F0B_AllJuv.diagnostic, option = "screeplot") # looks like 7
F0B_F1JuvAll.res             <- pcadapt(F0B_F1JALL_intersect.bed, K = 7) # assign
plot(F0B_F1JuvAll.res, option = "manhattan")

# look at both 
ggarrange(
  plot(F0B_AllJuv.diagnostic, option = "manhattan"),
  plot(F0B_F1JuvAll.res, option = "manhattan"), nrow = 2
)
```
...
par(mfrow = c(2, 2))
for (i in 1:4) 
  plot(F0B_F1JuvAll.res$loadings[, i], pch = 19, cex = .3, ylab = paste0("Loadings PC", i))
...
clump4me <- function(data, LD.thr, LD.K, LD.minMaf, outputfilename) {
        ...
        LD.window_loop      <- seq(50,50000, by=50) # run the loop for CPM from 1 - 10 (counts per million) 
...
test <- as.data.frame(clump4me(data         = F0B_AllJuveniles.bed,
...
F0B_F1JuvAll_LDdiagnostics <- pcadapt(F0B_AllJuveniles.bed, 
                                                K = 20, LD.clumping = list(size = 50000, thr = 0.1),
                                                min.maf= 0.05)
plot(F0B_F1JuvAll_LDdiagnostics, option = "screeplot") # Lets try and use K = 5 after SNP thinning 
F0B_F1JuvAll_LDres <- pcadapt(F0B_AllJuveniles.bed, 
                                        K =57, 
                                        LD.clumping = list(size = 50000, thr = 0.1), 
                                        min.maf= 0.05)
ggarrange( (plot(F0B_F1JuvAll_LDdiagnostics, option = "manhattan")),
           (plot(F0B_F1JuvAll_LDres, option = "manhattan")),nrow=2)
...
F0B_F1JALL_genind       <- vcfR2genind(F0B_AllJuveniles.vcf)
F0B_F1JALL_vcf_FILTERED <- F0B_AllJuveniles.vcf[!is.na(F0B_F1JuvAll_LDres$loadings[,1]),]
head(F0B_F1JALL_vcf_FILTERED,1)
F0B_F1JALL_genind_LD    <- vcfR2genind(F0B_F1JALL_vcf_FILTERED)
F0B_F1JALL_genind_LD

strata(F0B_F1JALL_genind_LD) <- F0BroodF1Juveniles_strata
setPop(F0B_F1JALL_genind_LD) <- ~Gen_Treatment
