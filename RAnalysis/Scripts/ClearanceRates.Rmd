...
Master_data <- read.csv(file="Data/Physiology/Clearance_rates/ClearanceRates_MasterData.csv", header=T,stringsAsFactors=FALSE, fileEncoding="latin1") # master data file
...
for (i in 1:nrow(loop_914)) {
  ...
  ClearanceRate.Ply          <- (V * ((ln( C0_Ply / Cend_Ply )/(t_Ply/60)) - A.ply)) / (dat$whole_Dry_weight[1]) # Tetraselmis cells mL-1  hr-1 mm shell length-1
  ClearanceRate.Ply_length   <- (V * ((ln( C0_Ply / Cend_Ply )/(t_Ply/60)) - A.ply)) / (dat$Length_um[1] / 1000) # Tetraselmis cells mL-1  hr-1 mm shell length-1
  ...
  ClearanceRate.Chaet        <- (V * ((ln( C0_Chaet / Cend_Chaet )/(t_Chaet/60)) - A.chaet)) / (dat$whole_Dry_weight[1]) # Chaetoceros cells mL-1  hr-1 mm shell length-1
  ClearanceRate.Chaet_length <- (V * ((ln( C0_Chaet / Cend_Chaet )/(t_Chaet/60)) - A.chaet)) / (dat$Length_um[1] / 1000) # Chaetoceros cells mL-1  hr-1 mm shell length-1
  ...
  ClearanceRate.Seston        <- (V * ((ln( C0_Seston / Cend_Seston )/(t_Seston/60)) - A.Seston)) / (dat$whole_Dry_weight[1]) # seston cells mL-1  hr-1 mm shell length-1
  ClearanceRate.Seston_length <- (V * ((ln( C0_Seston / Cend_Seston )/(t_Seston/60)) - A.Seston)) / (dat$Length_um[1] / 1000) # seston cells mL-1  hr-1 mm shell length-1
  
  dat2 <- dat[1,] %>%
    dplyr::select(c('Date','pH', 'Replicate', 'Fed_Unfed','Run', 'Plate', 'Number', 'Length_um', 'whole_Dry_weight')) %>% 
    mutate(Chaet_mL.hr.gWDW      = ClearanceRate.Chaet) %>% 
    mutate(Ply_mL.hr.gWDW        = ClearanceRate.Ply) %>% 
    mutate(Seston_mL.hr.gWDW     = ClearanceRate.Seston) %>% 
    mutate(Chaet_mL.hr.mmlength  = ClearanceRate.Chaet_length) %>% 
    mutate(Ply_mL.hr.mmlength    = ClearanceRate.Ply_length) %>% 
    mutate(Seston_mL.hr.mmlength = ClearanceRate.Seston_length)
    
  if (nrow(dat2) > 0) {
    FeedRate.table           <- data.frame(matrix(nrow = nrow(dat2), ncol = 15)) # create dataframe to save cumunalitively during for loop
    colnames(FeedRate.table) <- c('Date', 'ID', 'pH', 'Replicate', 'Fed_Unfed','Run', 'Plate', 'Number', 'Length_um', 
                                  'Chaet_mL.hr.gWDW', 'Ply_mL.hr.gWDW', 'Seston_mL.hr.gWDW',  
                                  'Chaet_mL.hr.mmlength', 'Ply_mL.hr.mmlength', 'Seston_mL.hr.mmlength') 
    ...
    FeedRate.table$Chaet_mL.hr.gWDW           <- dat2$Chaet_mL.hr.gWDW
    FeedRate.table$Ply_mL.hr.gWDW             <- dat2$Ply_mL.hr.gWDW
    FeedRate.table$Seston_mL.hr.gWDW          <- dat2$Seston_mL.hr.gWDW
...
Ply_FR_914_df      <- df_total.914 %>% filter(Ply_mL.hr.mmlength > 0) %>% mutate(type = 'ply') %>% 
  ...
  dplyr::rename(CR_mL.hr.WDW      = Ply_mL.hr.gWDW) %>% dplyr::select(!c('Chaet_mL.hr.gWDW','Seston_mL.hr.gWDW'))
Chaet_FR_914_df    <- df_total.914 %>% filter(Chaet_mL.hr.mmlength > 0) %>% mutate(type = 'chaet') %>% 
  ...
  dplyr::rename(CR_mL.hr.WDW      = Chaet_mL.hr.gWDW) %>% dplyr::select(!c('Ply_mL.hr.gWDW','Seston_mL.hr.gWDW'))
Seston_FR_914_df   <- df_total.914 %>% filter(Seston_mL.hr.mmlength > 0) %>% mutate(type = 'seston') %>% 
  ...
  dplyr::rename(CR_mL.hr.WDW      = Seston_mL.hr.gWDW) %>% dplyr::select(!c('Chaet_mL.hr.gWDW','Ply_mL.hr.gWDW'))
...
write.csv(ClearanceRate_Master, "C:/Users/samjg/Documents/Github_repositories/Airradians_multigen_OA/RAnalysis/Output/ClearanceRates/ClearanceRate_Master.csv")
