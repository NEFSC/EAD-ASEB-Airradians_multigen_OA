...
# kable(blanks_means) # View - notice some of the date/runs combos do not have BOTH treatmentents represented!
dups_to_add <- blanks_means[c(1, 16,19,20,23,26),] %>% # call the rows that do not have BOTH pH 7.5 and 8.0 represented (due to positive rates, bad data, etc)
              dplyr::mutate(pH = ifelse(pH == 8.0, 7.5,  ifelse(pH == 7.5, 8.0, NA))) # do a little conditional call to call the opposite treatment in this dups_to_add dataframe..
blanks_means <- rbind(blanks_means, dups_to_add) %>%  dplyr::arrange(Date_formatted, Run, pH)
