...
blanks_means <- blanks_all_raw %>% 
     ...
     dplyr::group_by(Date, pH, Run, filetype) %>% # grouped by date, pH, and instrument - similar amoung Runs
     dplyr::filter(Chamber_tank == "blank") %>% 
     ...
     dplyr::summarise(BLANK.mean_Lpc = mean(abs(Lpc)),
                      ...
                      BLANK.start.end_mean  = mean(Rate_mgO2_hour),
                      BLANK.start.end_sd  = mean(Rate_mgO2_hour),
                      n = n())
...
Resp.Master_1 <- merge(resp.data_merged, blanks_means, by=c("Date", "pH", "Run", "filetype")) %>% # NOTE: this repeats for every distinct length value
  dplyr::filter(!Date %in% '2/2/2022') %>% 
...
print(Resp.outliers) # 25  samples have negative values when corecting for the blank, measning the blank rate was GREATER than the sample resp value
...
# pCO2         1 0.0000009 8.900e-07   0.007  0.932
# Residuals   19 0.0022740 1.197e-04  
# Residuals   51 0.008346 0.0001637  
shapiro.test(residuals(LMmod_914_SL)) #  0.3276
leveneTest(LMmod_914_SL) # 0.4131
