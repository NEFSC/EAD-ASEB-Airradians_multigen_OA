...
Length_WholeDryWeight + geom_function(fun = function(x) 0.04*exp(x*0.15), colour = "blue") +
      coord_cartesian(ylim=c(0,5))


# Length_DryWeight_all_LOESS_obj <- stats::loess(Whole_animal_dry_weight_g ~ Shell_length_mm, data = (TDW_AFDW%>%dplyr::filter(Shell_length_mm > 0 & Whole_animal_dry_weight_g > 0)) )
# new_LOESS_data                 <- predict(Length_DryWeight_all_LOESS_obj)
# new_shellLength_obj <- (TDW_AFDW%>%dplyr::filter(Shell_length_mm > 0 & Whole_animal_dry_weight_g > 0))$Shell_length_mm
...
Length_TotalAFDW_facetted <- TDW_AFDW %>% 
