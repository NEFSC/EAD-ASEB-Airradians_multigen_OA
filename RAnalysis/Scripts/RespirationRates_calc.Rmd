...
resp.ref            <- read.csv(file="Data/Physiology/Respiration/Reference_master.csv", header=T) %>% 
                           dplyr::filter(!Date %in% "9/14/2021" | !Filename %in% 'Run_1_raw.txt')


# data to correct for length, dry weight, etc. 
lengths_juv_adults  <- read.csv(file="Data/Physiology/Respiration/Lengths_Condition_resp_clearance.csv", header=T)

lengths_spat        <- read.csv(file="Data/Physiology/Respiration/Lengths_larvae_spat_resp.csv", header=T) %>% 
...
Resp.Master_OM <- Resp.Master[!is.na(Resp.Master$Length_um),] %>% dplyr::filter(!resp_blankStand < 0) %>% # ommit respiration values that are positive  
  dplyr::mutate(volume = case_when(filetype == "LoLigo_data" & Date == '9/14/2021' & Channel == 'CH1' ~ 23.1,
                                   filetype == "LoLigo_data" & Date == '9/14/2021' & Channel == 'CH2' ~ 23.05,
                                   filetype == "LoLigo_data" & Date == '9/14/2021' & Channel == 'CH3' ~ 23.31,
                                   filetype == "LoLigo_data" & Date == '9/14/2021' & Channel == 'CH4' ~ 22.55,
                                   filetype == "LoLigo_data" & Date == '9/14/2021' & Channel == 'CH5' ~ 23.18,
                                   filetype == "LoLigo_data" & Date == '9/14/2021' & Channel == 'CH6' ~ 23.24,
                                   filetype == "LoLigo_data" & Date == '9/14/2021' & Channel == 'CH7' ~ 22.63,
                                   filetype == "LoLigo_data" & Date == '9/14/2021' & Channel == 'CH8' ~ 22.95,
      ...
      filetype == "LoLigo_data" & Date %in% c('9/22/2022') & Channel == 'CH1' ~ (236.70+18), 
      filetype == "LoLigo_data" & Date %in% c('9/22/2022') & Channel == 'CH2' ~ (239.88+18), 
      filetype == "LoLigo_data" & Date %in% c('9/22/2022') & Channel == 'CH3' ~ (228.63+18), 
      filetype == "LoLigo_data" & Date %in% c('9/22/2022') & Channel == 'CH4' ~ (234.35+18), 
      filetype == "LoLigo_data" & Date %in% c('9/22/2022') & Channel == 'CH5' ~ (224.31+18), 
      filetype == "LoLigo_data" & Date %in% c('9/22/2022') & Channel == 'CH6' ~ (229.07+18), 
      filetype == "LoLigo_data" & Date %in% c('9/22/2022') & Channel == 'CH7' ~ (223.11+18), 
      filetype == "LoLigo_data" & Date %in% c('9/22/2022') & Channel == 'CH8' ~ (225.59+18), 
...
print(Resp.outliers) # 32 samples have negative values when correcting for the blank, meaning the blank rate was GREATER than the sample resp value
View(Resp.Master_OM)
```
