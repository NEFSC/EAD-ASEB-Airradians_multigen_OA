...
# print(Resp.outliers) # 24 samples have negative values when correcting for the blank, meaning the blank rate was GREATER than the sample resp value
View(Resp.Master)
...
-   TDW b factor total = 0.62
-   TDW b factor pCO2 low = 0.639
-   TDW b factor pCO2 moderate = 0.628
-   TDW b factor pCO2 high = -0.165
...
-   Length b factor total = 1.71 (2.0 as of 5/24/2023 - added F3 spat) (1.85 as of 9/16/2023 - added lengths for 3/27/2023 F2s)
-   Length b factor pCO2 moderate = 1.69 (2.0 as of 5/24/2023 - added F3 spat) (1.9 as of 9/16/2023 - added lengths for 3/27/2023 F2s)
-   Length b factor pCO2 low = 1.74 (1.87 as of 5/24/2023 - added F3 spat) (1.77 as of 9/16/2023 - added lengths for 3/27/2023 F2s)
...
bTDW = 0.62 # review the outlier-omitted b factor plot in previous chunk
bLength = 1.85 # previously 1.71) # review the outlier-omitted b factor plot in previous chunk

# calc the mean for TDw and length
RR_master_OM$Dry_Tissue_weight <- as.numeric(RR_master_OM$Dry_Tissue_weight)

meanTDW      <- mean((RR_master_OM %>% 
                     dplyr::filter(!Dry_Tissue_weight %in% NA))$Dry_Tissue_weight) # 0.478945 g

meanLength   <- mean(RR_master_OM$Length_um/1000) # 15.75803 mm
...
RR_formatted_F1smaster <- RR_master_F1 %>% 
                              ...
                              dplyr::mutate(resp_mg_hr_bFactorNormTDW.1g = 
                                              (resp_mg_hr)*((1/Dry_Tissue_weight)^bTDW)) %>% # TDW b factor 
                              ...
                              dplyr::mutate(resp_umol_hr_bFactorNormTDW.1g = 
                                              (resp_umol_hr)*((1/Dry_Tissue_weight)^bTDW)) %>% # TDW b factor 
                              
                              # Length b factor - to mean length within generation
