...
Resp.Master_OM <- Resp.Master[!is.na(Resp.Master$Length_um),] %>% 
  ...
  dplyr::mutate(Biovol_length3 = (0.000198*(Length_mm^3)))  %>% 
  
  
  #  'actual_volume' as volume of vesst ('volume' in mL) - volume displaced by the animal (Biovol_estimate_mL3)
  dplyr::mutate(actual_volume = ifelse(is.na(Biovolume_g_in_sw), (volume - Biovol_estimate_VCap), # NA, calculate actual volume using the calculated biovolume using the length^3 regression EQ
