...
Resp.Master_OM <- Resp.Master[!is.na(Resp.Master$Length_um),] %>% 
  ...
  # Calculate biovolume using the volume of spherical cap and length measurements of scallops
  ...
  # Calculate biovolume for only absent data using the regression  
  # from length^3 to biovolume == (-0.00429)+(0.000198*x); x == length_mm^3
  dplyr::mutate(Biovol_length3 = (-0.00429)+(0.000198*(Length_mm^3)))  %>% 
  
  
  #  'actual_volume' as volume of vesst ('volume' in mL) - volume displaced by the animal (Biovol_estimate_mL3)
  dplyr::mutate(actual_volume =  (volume - Biovol_length3)) %>% # else use the biovolume measured
...
plot(Biovol_estimate_VCap~Biovol_length3, data = Resp.Master_OM %>% filter(Biovol_length3 < 0.01))
```
