...
resp.data_merged[211,c(1:6)] # 10/26/2022	CH3	run_2_raw.txt	0.04303898	-0.0047484	-0.0047484; data change Lpc to -0.1063
resp.data_merged[212,c(1:6)] # 10/26/2022	CH4	run_2_raw.txt	-0.003937913	-0.003744103	-0.003744103; data change Lpc to -0.0900
resp.data_merged[215,c(1:6)] # 10/26/2022	CH7	run_2_raw.txt	-0.002582753	-0.004017686	-0.004017686; data change Lpc to -0.1012
...
resp.data_merged[211,4] <- -0.1063
resp.data_merged[212,4] <- -0.0900
resp.data_merged[215,4] <- -0.1012
...
resp.data_merged[211,c(1:6)]  # -0.1063
resp.data_merged[212,c(1:6)]  # -0.09
resp.data_merged[215,c(1:6)]  # -0.1012
```
...
Resp.Master_OM <- Resp.Master[!is.na(Resp.Master$Length_um),] %>% dplyr::filter(!resp_blankStand < 0) %>% # ommit respiration values that are positive  
  dplyr::mutate(volume = 
            ...
            case_when(filetype == "LoLigo_data" & Date == '9/14/2021' ~ 1.7,
                                                   
                      filetype == "LoLigo_data" & Date %in% c('10/26/2021','2/2/2022','3/1/2022') & Channel == 'CH1' ~ 68.55323, 
                      filetype == "LoLigo_data" & Date %in% c('10/26/2021','2/2/2022','3/1/2022') & Channel == 'CH2' ~ 68.85583, 
                      filetype == "LoLigo_data" & Date %in% c('10/26/2021') & Channel == 'CH3' ~ 68.87473,
                      filetype == "LoLigo_data" & Date %in% c('10/26/2021','2/2/2022','3/1/2022') & Channel == 'CH4' ~ 68.95481,
                      filetype == "LoLigo_data" & Date %in% c('10/26/2021','2/2/2022','3/1/2022') & Channel == 'CH5' ~ 68.57288,
                      filetype == "LoLigo_data" & Date %in% c('10/26/2021','2/2/2022','3/1/2022') & Channel == 'CH6' ~ 68.01878,
                      filetype == "LoLigo_data" & Date %in% c('10/26/2021','2/2/2022','3/1/2022') & Channel == 'CH7' ~ 68.54551, 
                      filetype == "LoLigo_data" & Date %in% c('10/26/2021','2/2/2022','3/1/2022') & Channel == 'CH8' ~ 68.53297,
                      
                      filetype == "LoLigo_data" & Date %in% c('2/2/2022','3/1/2022') & Channel == 'CH3' ~ 68.87473,
                                                   
                                                   
                      filetype == "LoLigo_data" & Date %in% c('9/22/2022', '10/26/2022') & Channel == 'CH1' ~ (236.70+18), 
                      filetype == "LoLigo_data" & Date %in% c('9/22/2022', '10/26/2022') & Channel == 'CH2' ~ (239.88+18), 
                      filetype == "LoLigo_data" & Date %in% c('9/22/2022', '10/26/2022') & Channel == 'CH3' ~ (228.63+18), 
                      filetype == "LoLigo_data" & Date %in% c('9/22/2022', '10/26/2022') & Channel == 'CH4' ~ (234.35+18), 
                      filetype == "LoLigo_data" & Date %in% c('9/22/2022', '10/26/2022') & Channel == 'CH5' ~ (224.31+18), 
                      filetype == "LoLigo_data" & Date %in% c('9/22/2022', '10/26/2022') & Channel == 'CH6' ~ (229.07+18), 
                      filetype == "LoLigo_data" & Date %in% c('9/22/2022', '10/26/2022') & Channel == 'CH7' ~ (223.11+18), 
                      filetype == "LoLigo_data" & Date %in% c('9/22/2022', '10/26/2022') & Channel == 'CH8' ~ (225.59+18), 
                
                      
                                                   
                     filetype == "SDR_data" & Date %in% c('9/30/2021')~ 1.7,
                     
                     filetype == "SDR_data" ~ 0.08)) %>% # 24-well plate - 80 ul (0.08 mL)
  
  
  # Calculate biovolume using the volume pf spherical cap and length measurements of scallops
  ...
  #  'actual_volume' as volume of vesst ('volume' in mL) - volume displaced by the animal (Biovol_estimate_mL3)
  dplyr::mutate(actual_volume = volume - Biovol_estimate_mL3) %>% 
  ...
  dplyr::mutate(resp_mg_L_hr   = ( (abs(resp_blankStand)) *  # currently as just mg O2 per minute
                                      ...
                                      (actual_volume/1000) * # correct Liters - mg per L per min
...
print(Resp.outliers) # 27 samples have negative values when correcting for the blank, meaning the blank rate was GREATER than the sample resp value
View(Resp.Master_OM)
...
Resp.Master_OM[!is.na(Resp.Master_OM$Dry_Tissue_weight),] %>% 
                                dplyr::mutate(Dry_Tissue_weight = as.numeric(Dry_Tissue_weight)) %>% 
                                ggplot(aes(x = Dry_Tissue_weight, y = resp_umol_L_hr)) +
                                geom_smooth(method = "loess") +
                                theme_classic() +
                                ylab("resp_umol_L_hr") +
...
Resp.Master_OM$resp_umol_L_mm_Length_hr <- (Resp.Master_OM$resp_umol_L_hr / # multiple by 1000 to convert mmol/ O2 to umol/L O2
...
Resp.Master_OM$resp_umol_L_mg_TDW_hr <- (Resp.Master_OM$resp_umol_L_hr / # multiple by 1000 to convert mmol/ O2 to umol/L O2
              (as.numeric(Resp.Master_OM$Dry_Tissue_weight)*1000)) # normalize by Dry tissue weight  as to mg O2 L-1 g dry tissue weight-1


# (3) ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# NORMALIZED for whole Dry weight (WDW)
Resp.Master_OM$resp_umol_L_mg_WDW_hr <- (Resp.Master_OM$resp_umol_L_hr / # multiple by 1000 to convert mmol/ O2 to umol/L O2
              (as.numeric(Resp.Master_OM$whole_Dry_weight)*1000)) # normalize by Dry tissue weight  as to mg O2 L-1 g dry tissue weight-1
