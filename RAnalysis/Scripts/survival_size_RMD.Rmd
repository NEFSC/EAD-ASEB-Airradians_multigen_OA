...
Alldensity <- ggplot(data = ShellLength, aes(x=Length_mm)) + 
                geom_density(aes(x = Length_mm, group=Treatment, fill=Treatment, ..scaled..), alpha = 0.4, adjust=1.5) + 
                theme_bw() +
                facet_wrap( ~ Age_DPF) + 
                scale_fill_brewer(palette = "Set1")

density_217DPF <- ShellLength %>% 
                  dplyr::filter(Age_DPF %in% 217) %>% 
                  ggplot(aes(x=Length_um)) + 
                    geom_density(aes(x = Length_um, group=Treatment, fill=Treatment, ..scaled..), alpha = 0.4, adjust=1.5) + 
                    theme_bw() +
                    scale_fill_brewer(palette = "Set1")

pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_multigen_OA/RAnalysis/Output/Survival_Size/Juvenile_ShellLength_density.pdf"), width = 8, height = 6)
print(Alldensity) # print the model diagnostics 
dev.off()

pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_multigen_OA/RAnalysis/Output/Survival_Size/Juvenile_ShellLength_density_217DPF.pdf"), width = 8, height = 6)
print(density_217DPF) # print the model diagnostics 
dev.off()
```
...
TDW_AFDW$Gonad_proportion <- (TDW_AFDW$Dry_Gonad_weight_g / (TDW_AFDW$Dry_Gonad_weight_g +TDW_AFDW$Dry_Tissue_weight_g) ) * 100
Gonad_data                <- (TDW_AFDW %>% dplyr::filter(Dry_Gonad_weight_g > 0)) # not all dates contain data for gonad indices!
...
eq <- ddply(Gonad_data,(Gonad_data$Date_sampled),lm_eqn)
...
ggarrange(Allboxplot_gonad, Allboxplot_gonad_prop,Allregress_gonadDW_length)
...
ggarrange(Allboxplot_gonad, Allboxplot_gonad_prop,Allregress_gonadDW_length) # print the model diagnostics 
dev.off()
...
Alldensity_SDW <-TDW_AFDW[!is.na(TDW_AFDW$Dry_Shell_weight_g),] %>% dplyr::filter(!Date_sampled %in% '44,532.00') %>% # weird thing, omt this later!
...
Alldensity_TDW <- TDW_AFDW[!is.na(TDW_AFDW$Dry_Tissue_Tin_Weight_g),] %>% dplyr::filter(!Date_sampled %in% '44,532.00') %>% # weird thing, omt this later!
                      dplyr::filter(!Date_sampled %in% '3/2/2022') %>% # 3/2/22 are just the animals used for respiration rate measurements
...
CI_TDW         <- TDW_AFDW %>% dplyr::filter(!Condition_index_drywtRatio_Walne1976 < 0) %>% dplyr::filter(!Date_sampled %in% '44,532.00') %>% # weird thing, omt this later!
                      dplyr::filter(!Date_sampled %in% '3/2/2022') %>% # 3/2/22 are just the animals used for respiration rate measurements
...
CI_AFDW         <- TDW_AFDW %>% dplyr::filter(!Condition_index_TotalADFW_div_length < 0) %>% dplyr::filter(!Date_sampled %in% '44,532.00') %>% # weird thing, omt this later!
                      dplyr::filter(!Date_sampled %in% '3/2/2022') %>% # 3/2/22 are just the animals used for respiration rate measurements
