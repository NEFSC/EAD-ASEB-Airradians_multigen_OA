...
### summary:
* all
-   TDW b factor total = 0.704
* Generation
-  F1 TDW b factor  = 0.67
-  F2 TDW b factor  = 0.771
* treamtent
-   Low pCO2 TDW b factor  = 0.675
-   Moderate pCO2 TDW b factor  = 1.17
-   High pCO2 TDW b factor  = 1.28
*  treamtent x Generation
F1
-   Low pCO2 F1 TDW b factor  = 0.675
-   Moderate pCO2 F1 TDW b factor  = 0.662
F2
-   Low pCO2 F2 TDW b factor  = 0.665
-   Moderate pCO2 F2 TDW b factor  = 0.832
-   High pCO2 F2 TDW b factor  = 0.829
...
-   Length b factor total = 1.86 
Generation
-   Length b factor pCO2 F1 = 2.12
-   Length b factor pCO2 F2 = 1.55 
-   Length b factor pCO2 F3 = 3.52
Treatment
-   Length b factor pCO2 low = 1.9
-   Length b factor pCO2 moderate = 1.77 
-   Length b factor pCO2 high = 2.29
Generation * Treatment
F1
-   Length b factor pCO2 low = 2.12
-   Length b factor pCO2 moderate = 2.12 
F2
-   Length b factor pCO2 low = 1.59
-   Length b factor pCO2 moderate = 1.57 
-   Length b factor pCO2 high = 1.87 
F3
-   Length b factor pCO2 low = 2.7
-   Length b factor pCO2 moderate = 2.19 
-   Length b factor pCO2 high = 4.11 
...
bTDW = 0.67 # review the outlier-omitted b factor plot in previous chunk
bLength.all = 2.12 # review the outlier-omitted b factor plot in previous chunk

# assign bfactors by treatment
bTDW.low <-  0.675 #mean data by rep tank # 6.99 - with all data
bTDW.mod <-  0.662 #mean data by rep tank # 6.57 - with all data

bLength.low <-  2.12 #mean data by rep tank # 6.99 - with all data
bLength.mod <-  2.12 #mean data by rep tank # 6.57 - with all data
...
RR_formatted_F1smaster <- RR_master_F1 %>% 
                              ...
                              dplyr::mutate(
                                            ...
                                            resp_mg_hr_bFactorNormLength.MEAN = 
                                              case_when(
                                                pH  == 8.0 ~ (resp_mg_hr)*((meanLength/Length_mm)^bLength.low),
                                                pH  == 7.5 ~ (resp_mg_hr)*((meanLength/Length_mm)^bLength.mod)
                                              ), 
...
# call all bfactors (calc above!) - these are using all data
bTDW = 0.771 # review the outlier-omitted b factor plot in previous chunk
bLength.all = 1.55 # review the outlier-omitted b factor plot in previous chunk

# assign bfactors by treatment
bTDW.low <-  0.665 #mean data by rep tank # 6.99 - with all data
bTDW.mod <-  0.832 #mean data by rep tank # 6.57 - with all data
bTDW.high <-  0.829 #mean data by rep tank # 6.57 - with all data

bLength.low  <-  1.59 #mean data by rep tank # 6.99 - with all data
bLength.mod  <-  1.57 #mean data by rep tank # 6.57 - with all data
bLength.high <-  1.87 #mean data by rep tank # 6.57 - with all data
...
meanTDW    <- mean((RR_master_F2 %>% 
                     dplyr::filter(!Dry_Tissue_weight %in% NA))$Dry_Tissue_weight) # 0.4836937 g

meanLength <- mean(RR_master_F2$Length_um/1000) # 18.45981 mm
                              ...
                              dplyr::mutate(
                                            ...
                                            resp_umol_hr_bFactorNormTDW.MEAN = 
                                              case_when(
                                                pH  == 8.0 ~ (resp_umol_hr)*((meanTDW/Dry_Tissue_weight)^bTDW.low),
                                                pH  == 7.5 ~ (resp_umol_hr)*((meanTDW/Dry_Tissue_weight)^bTDW.mod),
                                                pH  == 7.0 ~ (resp_umol_hr)*((meanTDW/Dry_Tissue_weight)^bTDW.high)
                                              ),
                                            ...
                                            resp_mg_hr_bFactorNormLength.MEAN = 
                                              case_when(
                                                pH  == 8.0 ~ (resp_mg_hr)*((meanLength/Length_mm)^bLength.low),
                                                pH  == 7.5 ~ (resp_mg_hr)*((meanLength/Length_mm)^bLength.mod),
                                                pH  == 7.0 ~ (resp_mg_hr)*((meanLength/Length_mm)^bLength.high)
                                              ), 
                                            ...
                                            resp_umol_hr_bFactorNormLength.MEAN = 
                                              case_when(
                                                pH  == 8.0 ~ (resp_umol_hr)*((meanLength/Length_mm)^bLength.low),
                                                pH  == 7.5 ~ (resp_umol_hr)*((meanLength/Length_mm)^bLength.mod),
                                                pH  == 7.0 ~ (resp_umol_hr)*((meanLength/Length_mm)^bLength.high)
                                              ) )
