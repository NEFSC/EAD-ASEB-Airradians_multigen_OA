...
# Gen 1  ------------------------------------------- #
dim(DE_F1s_dir) # 169 DEGs
DE_F1s_dir.UPREG  <- DE_F1s_dir %>% dplyr::filter(up %in% TRUE)         # UPREG call - 105
DE_F1s_dir.DWNREG <- DE_F1s_dir %>% dplyr::filter(down %in% TRUE)       # DWNREG call - 64
# Gen 2  ------------------------------------------- #
dim(DE_F2s_dir) # 169 DEGs
DE_F2s_dir.UPREG  <- DE_F2s_dir %>% dplyr::filter(up %in% TRUE)         # UPREG call - 8
DE_F2s_dir.DWNREG <- DE_F2s_dir %>% dplyr::filter(down %in% TRUE)       # DWNREG call - 9
```
...
# Gene length 
GO_gene.length <- Airr_Cvirg_annotation %>% dplyr::mutate(length = V4-V3) %>%  dplyr::select(c("V1","length"))
