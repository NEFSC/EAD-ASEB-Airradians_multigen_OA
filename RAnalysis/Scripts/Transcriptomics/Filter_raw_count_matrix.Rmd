...
ncol(raw_counts.as.table) # 40 counting the transcript.ID that we want to keep! 
...
View(head(raw_counts.matrix))
View(head(CPM_matrix))

# CPM 3 
thresh.CPM3 <- CPM > 3 # Which values in myCPM are greater than 3?
...
percent_pass_3cpm <- paste( ( (nrow(raw_counts.as.table %>% 
                                     dplyr::filter(transcript_id  %in% (rownames(counts.matrix.3CPM))))) / 
                               nrow(raw_counts.as.table) * 
