...
filtered_counts_matrix_annotation  <-
  read.csv(file="../../Output/Transcriptomics/Filtered_counts_matrix/filter_3cmpm50perc_WITH_ANNOTATION.csv", 
                                               ...
                                               header=TRUE)  # %>% dplyr::select(!c(Ai19, Ai3))
counts_matrix <- 
  data.frame(filtered_counts_matrix_annotation[,-c(1,41:44)],#  [,-c(1,39:42)]if you run above dplyr::select(!c(Ai19, Ai3))
...
Exp.metadata.mtx_2 <- as.data.frame(Exp.metadata.mtx[match(colnames(counts_matrix),rownames(Exp.metadata.mtx)), ]) %>% dplyr::rename(pCO2 = 1)
...
# truncated the count matrix for just the F1s on 20211027
counts_matrix_F1 <- counts_matrix %>% dplyr::select(
...
# check for 'TRUE' in each - check before proceeding  design
Exp.metadata_F1.mtx_2 <- as.data.frame(Exp.metadata_F1.mtx[match(colnames(counts_matrix_F1),
                                                                 rownames(Exp.metadata_F1.mtx)), ]) %>%
                          dplyr::rename(pCO2 = 1)
all(rownames(Exp.metadata_F1.mtx_2) %in% colnames(counts_matrix_F1)) # should be TRUE
all(rownames(Exp.metadata_F1.mtx_2) == colnames(counts_matrix_F1)) # should be TRUE

# build dds
dds_F1 <- DESeqDataSetFromMatrix(countData = counts_matrix_F1,
...
# truncated the count matrix for just the F1s on 20211027
counts_matrix_F2 <- counts_matrix %>% dplyr::select(
  (exp_data %>% 
...
# check for 'TRUE' in each - check before proceeding  design
Exp.metadata_F2.mtx_2 <- as.data.frame(Exp.metadata_F2.mtx[match(colnames(counts_matrix_F2),rownames(Exp.metadata_F2.mtx)), ]) %>% dplyr::rename(pCO2 = 1)
all(rownames(Exp.metadata_F2.mtx_2) %in% colnames(counts_matrix_F2)) # should be TRUE
all(rownames(Exp.metadata_F2.mtx_2) == colnames(counts_matrix_F2)) # should be TRUE

# build dds
dds_F2 <- DESeqDataSetFromMatrix(countData = counts_matrix_F2,
...
dds.run_F1_df <- as.data.frame(assay(dds.run_F1))
chart.Correlation(dds.run_F1_df[,c(1:5,15:20)], histogram=TRUE, pch=19)
```
...
# What are the Protein names of these DEGs?
# first gather gene IDs for the DEGs (note: this is also used for the pheatmap in the transformed Exp heatmap)
numDEGs_padj_F2 # 17 - number of DEGs (from above) - use as integer in call below
...
# volcano plot 
png("../../Output/Transcriptomics/DESeq2/F1_DEGs-VolcanoPlot.png", 1000, 1000, pointsize=20)
...
rlog.heatmap <- pheatmap(topgenes_rlog.corrected_F1, 
                            ...
                            fontsize = 8,
                            legend = TRUE)
save_pheatmap(rlog.heatmap, filename = "../../Output/Transcriptomics/DESeq2/F1_rlog_heatmap.png") #Save pheatmap
