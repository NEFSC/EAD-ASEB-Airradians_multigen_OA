...
AOVdf_total <- AOVdf_total %>% dplyr::arrange(sort(Age))
...
```
# one sig effect on Age 167 (during the grow out >40 DPF)
...
# pCO2         2  27.12  13.561   8.204 0.00937 **
TukeyHSD(mod167)
#                         diff        lwr      upr     p adj
# 800 uatm-500 uatm  0.6472559 -1.8909023 3.185414 0.7627482
# 1200 uatm-500 uatm 3.4630915  0.9249334 6.001250 0.0104402
# 1200 uatm-800 uatm 2.8158356  0.2776775 5.353994 0.0310590
```
...
summary(mod36)
shapiro.test(resid(mod36)) # 5.529e-06 - NOPE!
leveneTest(mod36) # 0.4391 - YUP!

KWmod36 <- kruskal.test(resp_umol_hr_bFactorNormLength.MEAN ~ pCO2,data =  F2_RR_Length_Meanbytank_spat)
# Kruskal-Wallis chi-squared = 0.49658, df = 1, p-value = 0.481 # NOPE!
```
