...
# save
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_multigen_OA/RAnalysis/Output/Respiration/F2/F2_RR_LENGTH_bfactor_LowvMod.pdf"), width = 6, height = 8)
print(ggarrange(F2s.BOXPLOT_Length_bfactor.MEAN, F2_RR_Length_MeanSE, nrow = 2)) # print the model diagnostics 
dev.off()
...
for (i in 1:nrow(Ttest_Dates)) {
  ...
  data_loop     <- F2_RR_Length_Meanbytank %>% 
        ...
        # run all modles
        Ttestmod.eqvar      <- t.test( as.numeric(as.factor(data_loop$pCO2)), 
                                       data_loop$resp_umol_hr_bFactorNormLength.MEAN, 
                                       var.equal = TRUE)
        
        Ttestmod.noneqvar   <- t.test( as.numeric(as.factor(data_loop$pCO2)), 
                                       data_loop$resp_umol_hr_bFactorNormLength.MEAN, 
                                       var.equal = FALSE)
