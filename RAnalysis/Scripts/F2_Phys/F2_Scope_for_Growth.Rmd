...
F2_Masterfile_2.0 <- merge(Biodep_blanks,F2_Masterfile, by = c("Date", "pH")) %>% 
  dplyr::mutate(
                # RR
                ...
                ExcretionRate_mgNH4_bFactorNormTDW =
                  ((ExcretionRate_umol_hr_bFactorNormTDW/1000)*18.04), # convert umol to mg NH4
...
# stats
library(car)
F2_Masterfile_1_31_23 <- F2_Masterfile_2.0 %>% dplyr::filter(Date %in% '1/31/2023')
SGFmod_131 <- aov(lm(SFG ~ pCO2 , data =F2_Masterfile_1_31_23 ))
shapiro.test(resid(SGFmod_131)) # 0.7315

leveneTest(SGFmod_131)# 0.3329
summary(SGFmod_131)
#             Df Sum Sq Mean Sq F value  Pr(>F)   
# pCO2         2  46256   23128   8.255 0.00313 **
# Residuals   17  47629    2802  

leveneTest(SGFmod_131)# 0.3366
summary(SGFmod_131)
#             Df Sum Sq Mean Sq F value  Pr(>F)   
# pCO2         2  46256   23128   8.255 0.00313 **
# Residuals   17  47629    2802
...
# $pCO2
#                          diff       lwr       upr     p adj
# 500 μatm-1200 μatm  -1.480943 -77.02634  74.06446 0.9986068
# 800 μatm-1200 μatm 100.021812  24.47641 175.56721 0.0091223
# 800 μatm-500 μatm  101.502754  28.92108 174.08443 0.0060884
...
# 500 μatm     132 20.0 17     89.9      174  a    
# 1200 μatm    134 21.6 17     88.0      179  a    
# 800 μatm     234 20.0 17    191.4      276   b   
```


```{r, stats 2-23-23}

F2_Masterfile_2_23_23 <- F2_Masterfile_2.0 %>% dplyr::filter(Date %in% '2/23/2023')
SGFmod_223 <- aov(lm(SFG ~ pCO2 , data =F2_Masterfile_2_23_23 ))
shapiro.test(resid(SGFmod_223)) # 0.5899
leveneTest(SGFmod_223)# 0.3686
summary(SGFmod_223)
#             Df Sum Sq Mean Sq F value  Pr(>F)   
# pCO2         2   5933  2966.4   7.658 0.00425 **
# Residuals   17   6585   387.3 
TukeyHSD(SGFmod_223)
# $pCO2
#                         diff       lwr      upr     p adj
# 500 μatm-1200 μatm -31.15070 -59.24016 -3.06124 0.0286399
# 800 μatm-1200 μatm  10.42024 -16.56725 37.40772 0.5925516
# 800 μatm-500 μatm   41.57094  13.48147 69.66040 0.0039029
...
# pCO2      emmean   SE df lower.CL upper.CL .group
# 500 μatm    -4.2 8.03 17    -21.2     12.8  a    
# 1200 μatm   27.0 7.44 17     11.3     42.6   b   
# 800 μatm    37.4 7.44 17     21.7     53.1   b   
```


```{r, stats 3-27-23}

F2_Masterfile_3_27_23 <- F2_Masterfile_2.0 %>% dplyr::filter(Date %in% '3/27/2023')
...
F2_Masterfile_2.0$pCO2 <- factor(F2_Masterfile_2.0$pCO2, levels= c('500 μatm','800 μatm','1200 μatm'))

F2_SGF_Means_by_tank <- F2_Masterfile_2.0 %>% 
...
pdf("C:/Users/samjg/Documents/Github_repositories/Airradians_multigen_OA/RAnalysis/Output/F2_SFG.pdf", width = 6, height = 8)
