...
run_ttest <- function(datafilename,  outputfilename) {
            ...
            # run all modles
            Ttestmod.eqvar      <- t.test( as.numeric(as.factor(Data_all$INDvar)), 
                                           Data_all$DPvar, 
                                           alternative = "greater",
                                           var.equal = TRUE)
            
            Ttestmod.noneqvar   <- t.test( as.numeric(as.factor(Data_all$INDvar)), 
                                           Data_all$DPvar,
                                           alternative = "greater",
                                           var.equal = FALSE)
...
Proportion_Gonad_plot <- Proportion_master %>% 
                          ...
                          stat_summary(fun.min = function(x) mean(x) - sd(x)/sqrt(length(x)), 
                                        ...
                                        geom = 'errorbar', width = 0.25, size = 1,
                                        position = position_dodge2(width = 0.5)) + 
                          labs(title="Proportion Gonad to Total Tissue", x ="Age (dpf)", y = "Dry gonad (proportion)") +
                          theme_classic() +
                          theme(panel.grid.major = element_blank(), 
                                panel.grid.minor = element_blank(), 
                                axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=1),
