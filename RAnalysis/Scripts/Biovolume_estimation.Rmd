...
Gurr2021_size.relationships %>%
  dplyr::filter(!thickness_mm > 50) %>% 
   ggplot(aes(x=length.cubed, y=pre_biovolume_mL)) + 
   geom_point() +
   xlab("length (mm)") + 
   ggtitle("OTHER DATA: length v. biovolume") +
...
Length3.biovol <- Gurr2021_size.relationships[!is.na(Gurr2021_size.relationships$length.biov),] %>%
  dplyr::mutate(length_3 = summarySE(measurevar="length.biov")) # mean = 2.55471	stdev = 0.4976332	- highly variable relationship
...
lm(Biovol_estimate_mL3~Biovolume_g_in_sw, data= Reference_resp_sizeOM)

Reference_resp_sizeOM %>% 
  ggplot(aes(x = Biovolume_g_in_sw, 
             y =Biovol_estimate_mL3)) +
         geom_point(pch = 19, 
                    size=1) +
         theme_classic() + 
         #ylim(0, 0.2) +
...
Reference_resp_sizeOM[!is.na(Reference_resp_sizeOM$Biovolume_g_in_sw),] %>%
  dplyr::mutate(biovol_actual_calc_ratio = (as.numeric(Biovolume_g_in_sw))/(as.numeric(Biovol_estimate_mL3))) %>% 
  summarySE(measurevar="biovol_actual_calc_ratio")  # mean = 2.55471	stdev = 0.4976332	- highly variable relationship

#0.9958587 , 0.1392953

0.1392953/0.9958587 # 0.1398746
